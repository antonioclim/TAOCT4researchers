...
This UNIT explores advanced structures that enable researchers to tackle
...
The fundamental insight driving this UNIT is that the choice of data structure
...
understanding its limitations is essential. For strings, Python uses SipHash,
...
Many data structure operations exhibit variable cost. Hash table insertion is typically O(1), but occasionally triggers O(n) resizing. Evaluating such structures requires amortised analysis, which averages cost over sequences of operations.

The accounting method assigns credits to cheap operations. When insertion costs less than expected, the surplus credit is stored with the inserted element. When resizing occurs, accumulated credits pay for the expensive operation. If every operation deposits enough credit, the amortised bound holds.

Consider dynamic arrays (Python lists). Appending to a non-full array costs O(1). Appending to a full array triggers reallocation to double capacity, copying n elements—an O(n) operation. However, between doublings, we perform n cheap appends. The total cost for 2n operations is O(n) for copying plus O(n) for appends, yielding O(1) amortised per operation.

The potential method offers an alternative perspective. Define a potential function Φ mapping data structure states to non-negative reals. The amortised cost of operation i is:

    ĉᵢ = cᵢ + Φ(Dᵢ) - Φ(Dᵢ₋₁)

For hash tables, set Φ = 2n - m where n is the element count and m is capacity. Cheap insertions increase Φ by 2 (adding one element without resizing), whilst expensive resizing decreases Φ by the copying cost. The sum telescopes to O(1) amortised per insertion.

Understanding amortised bounds prevents misguided optimisation. A developer might cache hash values to avoid rehashing during resize, not realising that resizing already accounts for this cost. Conversely, ignoring worst-case bounds can cause latency spikes in real-time systems where even occasional O(n) operations are unacceptable.

### Space-Time Trade-offs

The fundamental trade-off pervading data structure design is space versus time. Additional memory often enables faster operations through precomputation, indexing or redundancy.

Suffix arrays exemplify this trade-off. Given a string of length n, a suffix array stores all suffixes in sorted order using O(n) space. It enables O(m log n) pattern matching where m is pattern length. Suffix trees achieve O(m) matching but require O(n) pointers—often 20n bytes versus n bytes for the array. The choice depends on query frequency and memory constraints.

Bloom filters represent an extreme point on this spectrum. By accepting false positives, they achieve sub-linear space for membership queries. A set of n elements requiring O(n) space with exact methods needs only O(n) bits with a Bloom filter for reasonable false positive rates—a compression factor of 32-64× for typical pointer-heavy set implementations.

Skip lists illustrate probabilistic space-time trade-offs. With probability p, each element promotes to the next level, yielding expected O(log n) levels and O(n) total space. Higher p means more levels (faster search, more space); lower p means fewer levels (slower search, less space). The standard choice p = 1/2 balances these concerns.

In research computing, space constraints often dominate. Processing terabyte-scale genomic data demands constant or near-constant space algorithms. Streaming algorithms sacrifice exactness for bounded memory, processing elements once without storage. The space-accuracy trade-off is quantifiable: Count-Min sketches provide (ε, δ)-approximations using O(1/ε × log(1/δ)) space independent of stream length.

### Choosing Between Trees and Hash Tables

Both balanced trees and hash tables support efficient insertion, deletion and search, yet their characteristics differ substantially. Understanding these differences guides selection for specific applications.

Hash tables provide O(1) expected operations but offer no ordering. Trees guarantee O(log n) worst-case with maintained order. When range queries or ordered iteration matter, trees are essential. When pure lookup dominates and ordering is irrelevant, hash tables excel.

Memory access patterns favour hash tables with open addressing for cache-friendly sequential probing. Tree traversal follows pointers, incurring cache misses at each level. For small datasets fitting in L2 cache, the difference may be negligible. For large datasets, cache effects can dominate asymptotic complexity.

Worst-case guarantees matter in adversarial settings. An attacker knowing the hash function can craft inputs causing O(n) lookups through collision chains. Balanced trees guarantee O(log n) regardless of input distribution. Cryptographic hash functions and universal hashing mitigate but do not eliminate this concern.

Concurrent access favours certain structures. Concurrent hash tables partition well, allowing lock-free or fine-grained locking. Balanced trees require careful coordination during rotations. Lock-free skip lists offer an alternative combining tree-like ordering with hash-table-like concurrency properties.
...
05UNIT applies these structures to scientific computing. Efficient data
structures enable large-scale simulations—Monte Carlo methods, ODE solvers
and agent-based models. The graph algorithms from this UNIT support agent
