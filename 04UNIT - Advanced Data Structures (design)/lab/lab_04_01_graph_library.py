...
PREREQUISITES
─────────────
- Week 3: Complexity analysis, Big-O notation, benchmarking
...
def astar(
    ...
    pq: list[tuple[float, float, N]] = [(heuristic(source), 0, source)]
    visited: set[N] = set()
    ...
    while pq:
        ...
        for edge in graph.edges_from(current):
            ...
            if new_g < distances[edge.target]:
                ...
                predecessors[edge.target] = current
                f_score = new_g + heuristic(edge.target)
...
def has_cycle(graph: AdjacencyListGraph[N, W]) -> bool:
    """
    Detect whether a directed graph contains cycles.

    Uses DFS with three colours:
    - WHITE (0): unvisited
    - GREY (1): being processed (on current stack)
    - BLACK (2): completely processed

    A cycle exists if we find a back edge (edge to a GREY node).
    ...
    Example:
        >>> g = AdjacencyListGraph[int, int](directed=True)
        >>> g.add_edge(1, 2)
        >>> g.add_edge(2, 1)  # Creates cycle
        >>> has_cycle(g)
        True
    """
    WHITE, GREY, BLACK = 0, 1, 2
    colour: dict[N, int] = {node: WHITE for node in graph.nodes()}

    def dfs_visit(node: N) -> bool:
        colour[node] = GREY
        ...
        for neighbour in graph.neighbours(node):
            if colour[neighbour] == GREY:
                ...
                return True
            if colour[neighbour] == WHITE and dfs_visit(neighbour):
                return True
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Week 4 Lab: Graph Library",
