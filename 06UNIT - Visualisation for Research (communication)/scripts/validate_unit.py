...
Usage:
    ...
    python validate_unit.py 14 --strict
"""
...
AI_FINGERPRINTS_TIER1 = [
    _t('del', 've'), _t('di', 've into'), _t('cru', 'cial'), _t('pi', 'votal'), _t('straight', 'forward'),
    _t('le', 'verage'), _t('ro', 'bust'), _t('sea', 'mless'), _t('cutting', '-edge'), _t('best ', 'practices'),
    _t('game', '-changer'), _t('em', 'power'), _t('har', 'ness'), _t('land', 'scape'), _t('para', 'digm'),
    _t('syn', 'ergy'), _t('hol', 'istic'), _t('eco', 'system'), _t('util', 'ize'), _t('faci', 'litate'),
    _t('bol', 'ster'), _t('fos', 'ter'), _t('under', 'score'), _t('re', 'alm'), _t('my', 'riad'), _t('ple', 'thora'),
    _t('corner', 'stone'), _t('encom', 'passes'), _t('intri', 'cacies')
]

# AI Fingerprint patterns (Tier 2 - contextual) (Tier 2 - contextual)
...
AI_FINGERPRINTS_TIER3 = [
    r'^' + _t('wel', 'come to'), r'^' + _t('in this ', 'comprehensive'), r'^' + _t('let ', 'me explain'),
    _t('happy ', 'coding'), _t('good ', 'luck!'), _t('have ', 'fun!'), _t('stay ', 'tuned'),
    _t('feel ', 'free to'), _t("don't ", 'hesitate'), _t('i ', 'hope this helps'),
    _t('thanks ', 'for reading')
]
...
def main():
    ...
    # Allow running from inside the UNIT root as well as from a parent directory.
    if unit_dirs:
        unit_path = unit_dirs[0]
    else:
        looks_like_unit = base_path.name.startswith(f"{args.unit}UNIT") or f"{args.unit}UNIT" in base_path.name
        has_root_markers = (base_path / "README.md").exists() and (base_path / "lab").is_dir()
        if looks_like_unit and has_root_markers:
            unit_path = base_path
        else:
