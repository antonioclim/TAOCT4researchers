...
Week 2, Lab 2: Design Patterns Catalogue
═══════════════════════════════════════════════════════════════════════════════
...
PREREQUISITES
─────────────
- Week 2, Lab 1: Protocol-based simulation framework
...
class Observer(Protocol[T]):
    """
    Protocol for observers that receive updates.
    
    Type parameter T represents the type of data received in updates.
    """
    ...
    def update(self, data: T) -> None:
        ...
        Args:
            data: The new data from the subject
        """
        ...
...
class MetricsCollector:
    """Observer that collects metrics history."""
    ...
    def get_timeseries(self, key: str) -> tuple[list[float], list[float]]:
        """
        ...
        values = [s.values.get(key, 0.0) for s in self.history]
        return times, values
...
class AgentFactory(Protocol):
    ...
    def create(
        ...
        """Create an agent at the given position."""
        ...
...
class AgentPopulation:
    ...
    def populate(
        ...
        for _ in range(n):
            ...
            agent = self._factory.create(self._next_id, (x, y))
...
def validated(
    ...
    """
    Decorator factory that validates the first argument.
    
    Args:
        validator: Function that returns True if input is valid
        error_msg: Message for ValueError if validation fails
        
    Returns:
        Decorator function
        
    Example:
        >>> @validated(lambda x: x > 0, "Input must be positive")
        ... def square_root(x: float) -> float:
        ...     return x ** 0.5
        >>> square_root(-1)  # Raises ValueError
    """
    def decorator(func: Callable[..., T]) -> Callable[..., T]:
        @wraps(func)
        def wrapper(*args: Any, **kwargs: Any) -> T:
            if args and not validator(args[0]):
                raise ValueError(f"{error_msg}: got {args[0]}")
...
def demo_observer() -> None:
    ...
    # Create subject
    subject: Observable[SimulationState] = Observable()
    
    # Create and subscribe observers
    console = ConsoleLogger(prefix="[SIM]")
    metrics = MetricsCollector()
    
    subject.subscribe(console)
    ...
    times, temps = metrics.get_timeseries('temperature')
    logger.info(f"Collected temperatures: {temps}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Week 2 Lab 2: Design Patterns Catalogue"
    )
