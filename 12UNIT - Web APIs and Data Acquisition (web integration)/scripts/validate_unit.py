...
Usage:
    ...
    python validate_unit.py 12
    python validate_unit.py 12 --strict
"""
...
class UnitValidator:
    ...
    def __init__(self, unit: str, base_path: Path, strict: bool = False):
        ...
        Args:
            unit: Two-digit UNIT number (e.g., '12')
    ...
    def scan_ai_fingerprints(self, content: str, filename: str) -> list[str]:
        ...
        for term in AI_FINGERPRINTS_TIER1:
            if term.lower() in content_lower:
                # Skip if it's a class name like RobustAPIClient
                if term == "robust" and "RobustAPIClient" in content:
                    continue
                # Skip if it's in a reference/citation
                if term == "best practices" and "Best practices for" in content:
                    continue
                findings.append(f"Tier1: '{term}'")
        
        # Tier 2: Pattern check
        for pattern in AI_FINGERPRINTS_TIER2:
            if re.search(pattern, content_lower):
                findings.append(f"Tier2: pattern '{pattern}'")
        
        # Tier 3: Opening/closing check
...
def main():
    ...
    if unit_dirs:
        ...
        parent_dirs = list(base_path.parent.glob(f"{args.unit}UNIT*"))
        if parent_dirs:
            unit_path = parent_dirs[0]
        else:
