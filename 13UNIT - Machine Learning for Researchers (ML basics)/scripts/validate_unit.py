...
Usage:
    ...
    python validate_unit.py 13
    python validate_unit.py 13 --strict
"""
...
# Note: 'paradigm' excluded as it's legitimate in ML contexts (learning paradigm)
# Note: 'robust' excluded as it's legitimate in statistical contexts (robust to outliers)
# Note: 'ecosystem' excluded as it's legitimate (scikit-learn ecosystem)
AI_FINGERPRINTS_TIER1 = [
    "delve", "dive into", "crucial", "pivotal", "straightforward",
    "leverage", "seamless", "cutting-edge", "best practices",
    "game-changer", "empower", "harness", "landscape",
    "synergy", "holistic", "utilize", "facilitate",
...
def main():
    ...
    parser.add_argument("unit", nargs="?", default="13", help="UNIT number (01-14)")
    ...
    if unit not in VALID_UNITS:
        print(f"Error: Invalid UNIT '{unit}'. Valid: 01-14")
        sys.exit(1)
    
    base_path = Path(args.path)
    unit_dirs = list(base_path.glob(f"{unit}UNIT*"))
    
    if unit_dirs:
        unit_path = unit_dirs[0]
    elif base_path.name.startswith(f"{unit}UNIT"):
        unit_path = base_path
    elif base_path.name == "scripts":
        unit_path = base_path.parent
    else:
        print(f"Error: Cannot find {unit}UNIT directory")
        sys.exit(1)
    
    validator = UnitValidator(unit, unit_path, args.strict)
