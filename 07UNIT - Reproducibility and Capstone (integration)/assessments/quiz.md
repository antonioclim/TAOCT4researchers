...
When setting random seeds for reproducibility in a Python project that uses both NumPy and the standard library, which approach is most reliable?

A) Only set `random.seed()` since NumPy inherits from it  
B) Only set `np.random.seed()` since it's more thorough  
