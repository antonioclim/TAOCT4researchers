...
def monte_carlo_integrate(
    ...
    Uses the estimator: I = (b-a) * mean(f(X_i)) where X_i ~ Uniform(a, b).
...
def stratified_sampling(
    ...
    # Var(I) = (width/K)² * Σ Var(f in stratum k) / n_k
...
def convergence_study(
    ...
    errors_list: list[float] | None = [] if true_value is not None else None
    standard_errors: list[float] = []
    
    for n in n_values:
        trial_estimates: list[float] = []
        trial_ses: list[float] = []
        ...
        estimates.append(float(mean_estimate))
        standard_errors.append(float(mean_se))
        if errors_list is not None and true_value is not None:
            errors_list.append(abs(float(mean_estimate) - float(true_value)))

    n_array = np.array(n_values, dtype=float)
    estimates_array = np.array(estimates, dtype=float)
    standard_errors_array = np.array(standard_errors, dtype=float)
    errors_array: FloatArray | None = None
    if errors_list is not None:
        errors_array = np.array(errors_list, dtype=float)

    # Fit convergence order: SE ~ n^(-α)
    # log(SE) = -α * log(n) + c
    log_n = np.log(n_array)
    log_se = np.log(standard_errors_array)
    ...
    return ConvergenceStudy(
        ...
        errors=errors_array,
        standard_errors=standard_errors_array,
...
def demo_variance_reduction() -> None:
    ...
    for name, method in methods.items():
        ...
        for _ in range(n_trials):
            result = method()
            errors.append(abs(float(result.estimate) - float(true_value)))
...
def demo_convergence() -> None:
    ...
    if study.errors is None:
        logger.info("No analytical error available.")
        return
