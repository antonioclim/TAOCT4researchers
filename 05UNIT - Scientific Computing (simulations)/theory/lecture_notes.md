...
By the Law of Large Numbers, Î → I as n → ∞. The Central Limit Theorem tells us the error decreases as O(1/√n), meaning quadrupling samples halves the error. Crucially, this rate holds regardless of dimension—making Monte Carlo uniquely suited to high-dimensional integrals where deterministic methods fail.
...
**Segregation index**: Average fraction of same-type neighbours across all agents. Ranges from 0 (perfect integration) to 1 (complete segregation).

**Polarisation**: For flocking, the magnitude of the average normalised velocity: |⟨v/|v|⟩|. Equals 1 when all agents move identically, 0 for random directions.

**Clustering coefficient**: Fraction of agent triads that form closed triangles in the interaction network.
