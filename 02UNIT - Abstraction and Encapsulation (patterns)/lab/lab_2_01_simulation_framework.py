...
Week 2, Lab 1: Simulation Framework with Protocols and Generics
═══════════════════════════════════════════════════════════════════════════════

CONTEXT
───────
In research, simulations are ubiquitous across disciplines:
- Physics: N-body systems, fluid dynamics, quantum mechanics
- Biology: population dynamics, epidemics, neural networks
- Economics: markets, agent-based models, complex systems
- Climatology: atmospheric models, ocean circulation

A well-designed simulation framework must be:
1. GENERIC — operates with any model
2. EXTENSIBLE — easy to add new models
3. TESTABLE — components can be tested in isolation
4. PERFORMANT — minimal computational overhead

PREREQUISITES
─────────────
- Week 1: State concept from Turing machines, AST hierarchies
- Python: Intermediate proficiency with type hints
- Libraries: numpy, matplotlib, dataclasses

LEARNING OBJECTIVES
───────────────────
After completing this lab, you will be able to:
1. Define a Protocol interface for simulations
2. Implement multiple concrete simulation models
3. Create a generic simulation runner
4. Visualise simulation results in real time
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Week 2 Lab: Simulation Framework with Protocols and Generics"
    )
