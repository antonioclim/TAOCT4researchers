...
Usage:
    ...
    python validate_unit.py 02
    python validate_unit.py 02 --strict
"""
...
def _s(*parts: str) -> str:
    """Join string fragments without leaving the full token in the source."""
    return "".join(parts)


# AI Fingerprint terms (Tier 1 - immediate flags)
#
# The validator checks for these terms in *other* files. In order to avoid the
# validator flagging itself, the tokens are assembled from fragments.
...
class UnitValidator:
    ...
    def __init__(self, unit: str, base_path: Path, strict: bool = False):
        ...
        Args:
            unit: Two-digit UNIT number (01-14)
            base_path: Path to the UNIT directory
            strict: If True, treat AI fingerprints as errors
        """
        self.unit = unit
    ...
    def scan_ai_fingerprints(self, content: str, filename: str) -> list[str]:
        """
        Scan content for AI fingerprints across all tiers.
        ...
        Returns:
            List of findings describing detected AI fingerprints
        """
        findings = []
        content_lower = content.lower()
        
        # Tier 1 - exact term matches
        for term in AI_FINGERPRINTS_TIER1:
            ...
            if matches:
                for m in matches:
                    ...
                    findings.append(f"Tier1: '{term}' at line {line_no}")
        ...
        for pattern in AI_FINGERPRINTS_TIER2:
            ...
            if matches:
                for m in matches:
                    line_no = content[:m.start()].count('\n') + 1
                    findings.append(f"Tier2: pattern at line {line_no}")
        
        # Tier 3 - openings/closings
        for pattern in AI_FINGERPRINTS_TIER3:
            if re.search(pattern, content_lower, re.MULTILINE):
                findings.append(f"Tier3: opening/closing pattern")
    ...
    def validate_structure(self) -> None:
        """Validate directory structure against canonical layout."""
        required_dirs = [
    ...
    def validate_html_naming(self) -> None:
        """Verify HTML files have UNIT prefix where appropriate."""
        html_files = list(self.base_path.rglob("*.html"))
        
        for path in html_files:
            # Only check HTML files in theory/ and assets/animations/
            rel_path = path.relative_to(self.base_path)
            if str(rel_path).startswith(("theory/", "assets/animations/")):
...
def main() -> None:
    ...
    if unit_dirs:
        ...
        print(f"Error: Cannot find {args.unit}UNIT directory in {base_path}")
        sys.exit(1)
    
    print(f"Validating: {unit_path}")
