...
In 1999, the Mars Climate Orbiter crashed because a function returned impulse in pound-seconds, but the caller expected newton-seconds. A more expressive type system would have prevented this error at compile time. This incident—resulting in the loss of a $327.6 million spacecraft—exemplifies how unit conversion errors propagate catastrophically through computational pipelines.

The insight underlying this exercise is that physical quantities possess *dimensions* (length, time, mass, etc.) that constrain valid operations. One cannot meaningfully add a distance to a duration; however, one can divide distance by duration to obtain velocity. A type system encoding these dimensional constraints catches physically meaningless operations before runtime—at the moment code is written, rather than when it produces nonsensical results.

The technique employed here—*phantom types*—uses type parameters that exist solely for static checking and carry no runtime representation. This approach generalises beyond physical units to any domain where certain combinations of values are semantically invalid.
...
In functional programming, the Result/Either monad enables composing operations that may fail, without explicit try/except at each step. This pattern, sometimes termed "railway oriented programming" (a metaphor introduced by Scott Wlaschin), treats computation as a track that may diverge onto either a success rail or a failure rail. Once on the failure rail, subsequent operations are bypassed until explicit recovery is attempted.

The advantage over exception-based error handling is explicitness: function signatures declare the possibility of failure, and the type checker ensures failures are handled. The `and_then` combinator (also known as `flatMap`, `bind`, or the monadic bind operator `>>=`) threads the success value through a chain of potentially-failing operations, short-circuiting on the first failure. This pattern proves particularly valuable in data processing pipelines where multiple validation and transformation steps must succeed for the overall computation to produce a valid result.
...
The Builder pattern enables step-by-step construction of complex objects. With the type state pattern, the compiler verifies that all mandatory fields are set before build. This eliminates a class of runtime errors—missing required configuration—by encoding construction state in the type system itself.

Traditional builders rely on runtime checks (throwing exceptions if required fields are unset) or return `Optional` types requiring null-handling at call sites. The type state approach shifts this verification to compile time: the `build()` method simply does not exist on builders in invalid states. The type system itself prevents construction of incomplete objects, providing a stronger correctness guarantee than documentation or runtime assertions.

This pattern finds application wherever objects have mandatory configuration that cannot be defaulted—API clients, database connections, HTTP requests, scientific experiment configurations. By making invalid states unrepresentable, the compiler becomes an ally in enforcing domain constraints.
...
The laboratory exercises introduced the `Simulable` Protocol as an abstraction over dynamical systems. This part asks you to demonstrate mastery of that abstraction by implementing a novel model from your research domain (or a domain of interest). The exercise serves dual purposes: it verifies comprehension of the protocol interface, and it illustrates how good abstractions enable code reuse across disparate domains.

The beauty of the `Simulable` abstraction lies in its generality. Whether modelling epidemics, ecosystems, economies, or physical systems, the core structure remains constant: a state evolving over time according to deterministic rules. By conforming to this interface, your model immediately gains access to the infrastructure built atop it—the `SimulationRunner`, observers, metrics collectors and visualisation components.
