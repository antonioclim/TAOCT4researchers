...
|-----------|---------------------|-----------|
| Multiple algorithms, same interface | Strategy | Encapsulates algorithm selection, promotes interchangeability |
| Notifying multiple listeners | Observer | Decouples event source from handlers, supports dynamic subscription |
| Complex object construction | Factory | Centralises creation logic, supports conditional instantiation |
| Managing object state transitions | State | Eliminates conditionals, localises state-specific behaviour |
| Adding behaviour dynamically | Decorator | Avoids class explosion, supports runtime composition |

### Decision Flow

When selecting a pattern, consider these questions in sequence:
1. **Does the problem involve varying algorithms?** → Consider Strategy
2. **Do multiple objects need updates when state changes?** → Consider Observer
3. **Is object creation complex or conditional?** → Consider Factory
4. **Does behaviour depend on internal state?** → Consider State
5. **Do you need optional or stackable features?** → Consider Decorator
