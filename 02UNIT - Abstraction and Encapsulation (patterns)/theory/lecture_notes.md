...
**Encapsulation** bundles data with the methods that operate on that data. More importantly, it restricts direct access to some components, preventing external code from depending on internal implementation details. The distinction between encapsulation (bundling) and information hiding (access restriction) is often elided in practice, though Parnas emphasised the latter as the more consequential concept.
