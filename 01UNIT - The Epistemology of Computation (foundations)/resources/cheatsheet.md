...
|----------|-------------|----------|
| Normal order | Reduce leftmost outermost redex first | Guarantees termination if possible |
| Applicative | Reduce leftmost innermost redex first | Eager evaluation (Python, Java) |
| Call-by-name | Substitute argument unevaluated | Lazy semantics |
| Call-by-value | Evaluate argument before substitution | Strict semantics |

**Theorem (Normalisation):** If a term has a normal form, normal-order reduction will find it.
...
**Configuration:** A triple (q, tape_contents, head_position) fully describing machine state at any instant. Configurations form a sequence during computation, with each transition producing a new configuration from the current one.
